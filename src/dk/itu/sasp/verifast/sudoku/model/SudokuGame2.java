...
public class SudokuGame2 {
	...
	// @ predicate isValidHorizontalMove(int y) = N |-> ?n &*& game |-> ?g &*&
	// mainY |-> y &*& array_slice(g,n*y,n*(y+1),_);

	// @ predicate isValidMove(int x, int y, int number) = mainX |-> x &*& mainY
	// |-> y &*& mainNumber |-> number &*& isValidHorizontalMove(y);
	// TODO - add isValidVerticalMove + isValidBlockMove predicates to
	// isValidMove predicate
	// @predicate verticalElements(int x) = game |-> ?g &*& N |-> ?n &*& mainX
	// |-> x &*& g[x] |-> _ &*& g[x+n] |-> _ &*& g[x+n+n] |-> _ ;
	// @predicate fields() = N |-> ?n &*& game |-> ?g &*& n*n>0 &*& mainNumber
	// |-> ?mn &*& mainX |-> ?mx &*& mainY |-> ?my;
	// @predicate array_access(int[] g, int n) = game |-> g &*& N |-> n &*&
	// array_slice(g,0,n*n,_);
	...
	/*
	 * //Methods for determining if a move is valid private boolean
	 * isValidMove(int x, int y, int number) //@ requires
	 * isValidMove(x,y,number); //true //@ ensures isValidMove(x,y,number); {
	 * return isValidHorizontalMove(y, number); && isValidVerticalMove(x,
	 * number) && isValidBlockMove(x, y, number);
	 * 
	 * }
	 */
	...
	// @ requires N |-> ?n &*& game |-> ?g &*& mainY |-> y &*&
	// array_slice(g,n*y,n*(y+1),_) &*& 0<y &*& y<10;
	// @ ensures result ? isValidHorizontalMove(y) : !result;
	{
		for (int i = y * N; i < (y + 1) * N; i++)
		// @ invariant N |-> n &*& game |-> g &*& mainY |-> y &*& i >= n*y &*&
		// array_slice(g,n*y,n*(y+1),_);
		{
			if (game[i] == number) {
				return false;
			}
	...
	// @ ensures true;
	{
		...
		// @ invariant blockIndicies!=null &*& game |-> g &*& i>=0 &*&
		// array_slice(g,0,81,_) &*& array_slice(blockIndicies, 0,
		// blockIndicies.length,_);
		{
	...
	// Returns the index in one-dimensional array corresponding to x and y
	// coordinates
